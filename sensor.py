    ...
    async def _load_statistics ():
        ...
        for _aggr_method in ["month", "day", "hour"]:
            ...
            for x in _stats:
                ...
                _sum = 0
                for i in _data:
                    date = dt_util.as_local(dt_util.parse_datetime(i['start'])).isoformat()
                    if date not in stats[_aggr_method]:
                        stats[_aggr_method][date] = {'datetime': date, 'value_kWh': 0, 'value_p1_kWh': 0, 'value_p2_kWh': 0, 'value_p3_kWh': 0}
                    for x in ["total", "p1", "p2", "p3"]:
